...
unsigned long reportInterval = 10000;  // 100 Hz (in microseconds)
...
void loop() {
  ...
  if (imu.wasReset()) {
    Serial.println("Sensor reset detected! Reinitializing reports...");
    setReports();
  }
  ...
  if (imu.getSensorEvent(&sensorValue)) {
    if (sensorValue.sensorId == SH2_ROTATION_VECTOR) {
      ...
      unsigned long timestamp = millis();  // time since boot (ms)

      char quat_msg[128];
      snprintf(quat_msg, sizeof(quat_msg),
               "IMU 1 Quaternion: %.4f, %.4f, %.4f, %.4f, %lu",
               real, i, j, k, timestamp);
