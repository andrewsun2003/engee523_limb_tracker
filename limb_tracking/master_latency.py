...
def listen_udp():
    global quat1, quat2, quat3, esp_time_offset
    while True:
        data, _ = sock.recvfrom(1024)
        msg = data.decode().strip()
        now_pc_time = time.time() * 1000  # PC time in ms

        try:
            if "Quaternion:" in msg:
                label = msg.split("Quaternion")[0].strip()
                parts = msg.split(":")[1].strip().split(",")
                quat = [float(p.strip()) for p in parts[:4]]
                timestamp_esp = int(parts[4].strip())

                if esp_time_offset is None:
                    esp_time_offset = now_pc_time - timestamp_esp

                timestamp_pc_est = esp_time_offset + timestamp_esp
                latency = now_pc_time - timestamp_pc_est
                print(f"[LATENCY] {label} Delay: {latency:.2f} ms")
